...
module.exports = function pinoPrint (config) {
  ...
  const print = new Print({
    ...config,
    ...config.ansi ? {} : { chalkLevel: 0 }
  })

  // Abstract the write function so that all ANSI escape sequences are stripped
  // if they're not enabled.
  let write = line => print.write(stripAnsi(line))
  if (config.ansi) {
    write = print.write
  }
